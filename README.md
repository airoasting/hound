...
미리 깔아 둘 것도 없습니다. 유튜브 기간별 조회수 순위 집계에 쓰는 `yt-dlp`(키 불필요)는 필요한
순간에 스크립트가 알아서 설치합니다. 유튜브를 쓰지 않으면 그마저도 깔리지 않습니다.

손이 필요한 건 하나뿐입니다. 팟캐스트 오디오 전사에는 Whisper 계열 도구가 필요한데, 모델 용량이
커서 자동으로 깔지 않습니다. 그 기능을 쓸 때만 직접 준비하면 됩니다.
...
붙이고, 각 채널을 다루는 방법은 채널별 참조 문서를 따릅니다.
...
- 최초 공개.
- 16개 채널 라우터: 네이버, 유튜브, X, 쓰레드, 인스타그램, 페이스북, 레딧, 링크드인, GitHub, RSS,
  빌리빌리, 샤오홍슈, 슈에추, V2EX, 팟캐스트, 웹.
- 사냥개 추적 프로토콜 4단계(Scent, Track, Hold the line, Retrieve)와 5단계 에스컬레이션.
- 유튜브 기간·순위 집계 엔진(`yt_collect.py`)과 리포트 생성(`yt_render.py`).
- 검색 결과 저장(`save_result.py`): `output/{날짜}_{순번}/`에 `result.md`와 `result.html`.
- 자료별 2~3문장 요약과 의미 있는 숫자(팔로워·회원 수, 조회수 등) 회수 규칙.
- 기본 검색은 설치와 API 키 없이 동작.
